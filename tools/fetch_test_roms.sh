...
# DMA busy flags, and port access. Self-reporting, so a --shot PNG is the
# whole verdict. http://nemesis.hacking-cult.org/MegaDrive/Roms/Test/Mine/VDP/
set -eu
cd "$(dirname "$0")/.."
...
if [ ! -f "$vdp_out" ]; then
    # No automated test needs this ROM yet, so a runner without unzip is not a
    # reason to fail the build the Cave Story fetch above is guarding.
    if ! command -v unzip >/dev/null; then
        echo "unzip not found; skipping $vdp_out" >&2
        exit 0
    fi
    # One person's HTTP site with no mirror, and CI does not use this ROM, so
    # an outage there must not turn every pull request red.
